# for part 2, we just need to reverse the algorithm : start from the 'E' point and search downward until reaching at least one point
# at elevation 'a'.
# rules of propagation are also reverse :
# move at most 1 level downward
# move any value upward.
...
# find all points at elevation a that have been reached and sort the list to find the minimum number of steps
indexEndPoint = inputMatrix |> List.flatten |> Enum.find_index(&(&1 == ?E))
{_, shortestpathfromminimumelevation} = result
  |> List.flatten()
  |> Enum.filter(fn {elevation, steps} -> elevation == ?a && steps >= 0 end)
  |> Enum.sort(fn {_, step1}, {_, step2} -> step1 < step2 end )
  |> IO.inspect(label: "points at lowest elevation")
  |> List.first(0)

  IO.puts("shortest path from 'a' elevation to the top #{shortestpathfromminimumelevation}")
