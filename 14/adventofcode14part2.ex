...
# for part 2 we need to extend the width so as to fit a heap of sand that is twice as large
# as it is high (45° slope). So it has to be 2x(ymax+1) wide
# xmin and xmax will need to change to xmin-ymax-1 and xmax+ymax+1
air_row = (xmin-ymax-1)..(xmax+ymax+1) |> Map.new(&({&1,:air}))
rock_row = (xmin-ymax-1)..(xmax+ymax+1) |> Map.new(&({&1,:rock}))
...
# add row of rocks at ymax+2
rock_grouped_by_rows = rock_grouped_by_rows |> Map.put(ymax+2, rock_row)

# range 1..28000 has been adjusted after finding the answer of 27936

sand_heap = for n <- 1..28000, reduce: rock_grouped_by_rows do
  rock_matrix ->
    {x,y} = Main.drop_sand(rock_matrix, {500,0})
    if y>0 do #condition changed for part2
      Main.update_matrix(rock_matrix, {x,y})
    else
      IO.puts("heap reached level 0 at #{n}")
      rock_matrix
    end
end
